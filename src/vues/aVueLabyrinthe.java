...
public class aVueLabyrinthe implements Serializable{
    ...
    private static aVueLabyrinthe instance = null;
	public static aVueLabyrinthe getInstance() throws SlickException {
		if (instance == null) {
			instance = new aVueLabyrinthe();
		}
		return instance;
	}

    private aVueLabyrinthe() throws SlickException {
        aElementDecor.mettreForet();
    ...
    public void render(GameContainer container, Graphics g,int xMin , int xMax , int yMin , int yMax){
        ...
        for(int i = hauteurMin ; i < hauteur && i <= hauteurMax ;i++){
            for(int j = largeurMin ; j < largeur && j <= largeurMax ; j++){
                g.drawImage(aElementDecor.getChemin(),xActu,yActu);
                if(mur[j][i]!= null){
                    g.drawImage(aElementDecor.getMur(),xActu,yActu);
                }
                xActu += LARGEUR_MUR;
            }
